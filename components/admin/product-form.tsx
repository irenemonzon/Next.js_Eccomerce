...
import { productDefaultValue } from "@/lib/contants"
...
const ProductForm = ({type,product,productId}:{
    ...
    type Insert = z.infer<typeof insertProductSchema>;
    type Update = z.infer<typeof updateProductSchema>;

    const isUpdate = type === 'Update';

    const form = useForm<Insert | Update>({
    resolver: zodResolver(isUpdate ? updateProductSchema : insertProductSchema),
    defaultValues: isUpdate
        ? 
        ({ ...(product ?? {}), id: product?.id ?? productId ?? '' } as Update)
        : (productDefaultValue as Insert),
    });
                ...
                {/*Slug*/}
                 <FormField
                    ...
                    render={({field}:{field:ControllerRenderProps<z.infer<typeof insertProductSchema>,'slug'>;})=>(
                        <FormItem className="w-full">
                            <FormLabel className="pt-2">Slug</FormLabel>
                            <FormControl>
                                <div className="flex">
                                    <Input placeholder="Enter slug"{...field}/>
                                    <Button type="button" className="bg-gray-500 hover:bg-gray-600 text-white px-4 py-1 ml-2"
                                    onClick={()=>{
                ...
                <FormField
                    ...
                    render={({field}:{field:ControllerRenderProps<z.infer<typeof insertProductSchema>,'price'>;})=>(
                        <FormItem className="w-full">
                            ...
                            <FormControl>
                                <Input placeholder="Enter products price"{...field}/>
                            </FormControl>
                            <FormMessage/>
                        </FormItem>
                    )}
